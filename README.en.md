...
| QR conversion | Implemented, under continuous validation | Use the F4 global shortcut to generate a QR image from the latest internal text or decode one from an image; the result is saved to internal history, system clipboard sync is attempted, and a non-activating top-right toast reports the outcome |
| Theme and general settings | Implemented, validation ongoing | Light/dark themes, preset and color-picker custom accents, motion/transparency preferences; main-window local PNG/JPEG/WebP image skins with fit, dim, blur, and panel-opacity controls; tray behavior, autostart, data-directory migration, and local diagnostics preference |
