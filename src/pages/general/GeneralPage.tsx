    ...
    <PageScaffold title="常规" description="应用启动、数据目录、更新和基础行为设置。">
      <div className={styles.generalGrid}>
        <SettingsCard fill>
          ...
          <SwitchRow title="开机自启动" description="登录 Windows 后自动启动后台服务" checked />
          <SwitchRow title="启动时最小化到托盘" description="不主动打断当前桌面" checked />
          <SwitchRow title="关闭按钮最小化到托盘" description="保持后台快捷键和剪贴板服务" checked />
        </SettingsCard>
        <SettingsCard fill>
          <SectionTitle>数据与隐私</SectionTitle>
          <FieldRow label="数据目录">
            <TextField value="%APPDATA%\\OpenDeskTools" disabled />
          </FieldRow>
          <SwitchRow title="本地保存设置" description="设置和历史数据只保存在本机" checked />
          <SwitchRow title="崩溃日志" description="仅保存本地诊断日志" checked={false} />
        </SettingsCard>
        <SettingsCard fill>
          <SectionTitle>更新</SectionTitle>
          <SwitchRow title="自动检查更新" description="启动后检查是否有新版本" checked />
          <FieldRow label="当前版本">
            <TextField value="0.1.0" disabled />
          </FieldRow>
